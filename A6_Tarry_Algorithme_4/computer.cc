    ...
    // l'iniateur d'index 0
       if(getIndex() == 0)
       {
           for(int i = 0; i < gateSize("gate$o"); i++)
           {
               ...
               //Vos.insert(gate("gate$o",i)->getPathEndGate()->getOwnerModule()->getIndex());
               Vos.insert(i);
           }
           // Chosit un Voisin pour l'envoyée le Token
           int To = intuniform(0,gateSize("gate$o"));
           //Vos.erase(gate("gate$o",To)->getPathEndGate()->getOwnerModule()->getIndex());
           Vos.erase(To);

           iniator = true;   // l'iniaruer est le pere de lui meme
           done = true;
